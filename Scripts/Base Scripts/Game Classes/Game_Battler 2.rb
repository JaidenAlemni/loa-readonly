...
class Game_Battler
  ...
  # This is the animation ID of the one that should currently be displaying
  # States are all cycled through
  #--------------------------------------------------------------------------
  def state_animation_id
    # If the battler is casting a spell
    # if self.skilling?
    #   return CASTING_ANIM
    # end
    # If the actor has no states, or they're dead
    if @states.empty? || @states.include?(1)
      # Clear the animation state array
      @anim_states = []
      # No animation
      return 0
    end
    # If the current animation in the state array is displaying
    @state_frame -= 1 if @state_frame > 0
    return @state_animation_id if @state_frame > 0
    # Create an array of states to be displayed
    if @anim_states.empty?
      @states.each do |state_id|
        if $data_states[state_id].animation_id > 0 && !$data_states[state_id].has_extension?(:NOLOOPANIM)
          @anim_states << state_id
        end
      end
    end
    # Grab the next state in the list
    now_state = @anim_states.shift
    # No state to display
    return 0 if now_state.nil?
    # Set the state ID to this state
    @state_animation_id = $data_states[now_state].animation_id
    return 0 if $data_animations[@state_animation_id].nil?
    @state_frame = $data_animations[@state_animation_id].frame_max + STATE_LOOP_DELAY
    return @state_animation_id
  end
  #--------------------------------------------------------------------------
  # Return battler states array
  #--------------------------------------------------------------------------
  def battler_states
    bat_states = []
    for i in self.states # State IDs array
  ...
  #--------------------------------------------------------------------------
  # * Natural Removal of States (called up each second)
  #--------------------------------------------------------------------------
    ...
    for i in plus_state_set
      ...
      unless self.state_guard?(i)
        ...
        # If this state is not full
          ...
          if rand(100) < [0,98,80,60,40,20,0][self.state_ranks[i]]
            # Set state change flag
            @state_changed = true
            # Add a state
            add_state(i)
