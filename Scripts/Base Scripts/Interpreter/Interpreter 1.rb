...
class Interpreter
  ...
  def setup_starting_event
    ...
    # Call and run Init event first if it exists
    init_event = $game_map.init_event
    if init_event && init_event.starting?
      puts "setup init event"
      # Lock event
      init_event.clear_starting
      init_event.lock
      # Setup 
      setup(init_event.list, init_event.id)
      return
    end
    ...
    for event in $game_map.events.values
      ...
      if event.starting
        # If not auto run, pp or touch trigger
        if event.trigger < 3
          # Clear starting flag
          ...
          event.lock
          # Remove trigger flag
          event.trigger_on = false if event.trigger < 2
        end
        # Set up event
  ...
  def setup_choices(parameters)
    ...
    $game_temp.choices_text = parameters[0].dup
    # Add a flag in the string
