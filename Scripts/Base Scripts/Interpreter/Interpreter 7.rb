...
class Interpreter
  ...
  def command_339
    ...
    iterate_battler(@parameters[0], @parameters[1]) do |battler|
      ...
      if battler.exist?
        # Set action
        # Adjust parameter to symbol
        battler.current_action.kind = @parameters[2]
        if battler.current_action.kind == :basic
  ...
  # * Script
  # - 
  # Edited by jaiden -- call must return false and advance the index 
  # in order for the interpreter to process the next item properly
  #--------------------------------------------------------------------------
  def command_355
    ...
    if result == false
      # Advance index
      @index += 1
      # Break out of the interpreter loop
      return false
    end
    # Continue
    return true
