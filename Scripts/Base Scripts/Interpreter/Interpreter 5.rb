...
class Interpreter
  ...
  def command_201
    ...
    # Adjust for PM
    $game_temp.player_new_x = $game_temp.player_new_x * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE / 2
    $game_temp.player_new_y = $game_temp.player_new_y * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE - 2
    # Advance index
    @index += 1
    # If fade is set
    if @parameters[5] == 0
  ...
  def command_202
    ...
    # If appointment method is [direct appointment]
    if @parameters[1] == 0
      # Set character position (adjusted for PM)
      character.moveto(@parameters[2] * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE/2, @parameters[3] * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE - 2)
    # If appointment method is [appoint with variables]
    elsif @parameters[1] == 1
      # Set character position
      character.moveto($game_variables[@parameters[2]] * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE/2, $game_variables[@parameters[3]] * Game_Map::TILE_SIZE + Game_Map::TILE_SIZE - 2)
  ...
  def command_204
    ...
    when 1  # fog
      # Default to first fog for compatibility's sake
      $game_map.fogs[0][:name] = @parameters[1]
      $game_map.fogs[0][:hue] = @parameters[2]
      $game_map.fogs[0][:opacity] = @parameters[3]
      $game_map.fogs[0][:blend_type] = @parameters[4]
      $game_map.fogs[0][:zoom] = @parameters[5]
      $game_map.fogs[0][:sx] = @parameters[6]
      $game_map.fogs[0][:sy] = @parameters[7]
    when 2  # battleback
