...
class Interpreter
    ...
    loop do
      ...
      # If event command is not on the second line or after
        ...
        if @list[@index+1].code == 102
          # No longer allow splitting choices into their own window;
          # They will be forced to fit on the same window for readability
          # and split into 2 columns
          # Advance index
          @index += 1
          # Choices setup
  ...
  #--------------------------------------------------------------------------
  # * Conditional Branch
  # (Modified for additional accessory)
  #--------------------------------------------------------------------------
  def command_111
      ...
      if actor != nil
        ...
        when 4  # armor
          result = (actor.armor1_id == @parameters[3] or
                    ...
                    # If reinstating armor5, remove  V this parentheses
    ...
    when 6  # character
      ...
      if character != nil
        # Modify direction check to include diagonals
        result =
          case @parameters[2]
          when 2 # Down
            [1,2,3].include?(character.direction)
          when 4 # Left
            [1,4,7].include?(character.direction)
          when 6 # Right
            [3,6,9].include?(character.direction)
          when 8 # Up
            [7,8,9].include?(character.direction)
          end
      end
    when 7  # gold
      if @parameters[2] == 0
